 ...
 *
 * Der Zufallsgenerator steht hier noch einmal, obwohl das Doppelkopf-Modul
 * denselben hat. Das ist Absicht: Ein Spielmodul ist eine eigenstaendige
 * Bibliothek ohne Abhaengigkeit auf ein anderes Spiel. Wanderte der Generator
 * in ein gemeinsames Paket, aenderte eine Verbesserung dort die Gaben BEIDER
...
export interface Deal {
  /** Haende in der Reihenfolge der uebergebenen Sitze. */
  readonly hands: readonly (readonly Card[])[];
  /**
   * Oberste Karte des Rests, die den Trumpf bestimmt. Null, wenn das Blatt
   * restlos verteilt wurde - das ist genau die letzte Runde einer vollen
   * Partie und heisst: kein Trumpf.
   */
  readonly upcard: Card | null;
}

/**
 * Gibt jedem Sitz `handSize` Karten und deckt danach die naechste Karte auf.
 */
export function deal(seats: number, handSize: number, seed: Seed): Deal {
  const shuffled = shuffle(createDeck(), makeRng(seed));
  const hands: Card[][] = Array.from({ length: seats }, () => []);
  let index = 0;
  for (let round = 0; round < handSize; round++) {
    for (let seat = 0; seat < seats; seat++) {
      hands[seat]!.push(shuffled[index++]!);
    }
  }
  return { hands, upcard: shuffled[index] ?? null };
}
