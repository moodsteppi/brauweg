 ...
 * Felder wie im Spiel), einmal MIT Störfeldern und einmal OHNE (die drei
 * Störfelder werden am Lochbeginn aus den Feldern genommen — sonst dieselbe
 * Partie). Je Stufe:
 *
 *   - Schläge je Ball und Loch, mit und ohne
 *   - Abstand Erster–Letzter am Matchende, mit und ohne (das Gummiband)
 *   - ausgelöste Störschläge je Loch, nach Art, und wie viele ein Schild abfing
 *   - Störfelder je Loch: gelegt, eingesammelt
...
import { felderVon } from '../src/minispiele/golf/powerup';
import { ausloesenErlaubt, botStoerschlag, istStoerart } from '../src/minispiele/golf/stoerschlag';
...
function spiele(saat: number, stufe: Botstufe, mitStoer: boolean, m: Messung): void {
  const karten: Karte[] = [];
  ...
  while (!z.fertig && deckel < 200_000) {
    ...
    for (let s = 0; s < SITZE && s < z.baelle.length; s += 1) {
      ...
      if (mitStoer && istStoerart(z.baelle[s].halt) && haltVorher[s] !== z.baelle[s].halt) m.eingesammelt += 1;
    }
  ...
  m.abstand += Math.max(...summe) - Math.min(...summe);
}
...
console.log('Stufe       Schläge/Ball/Loch  Abstand 1.–4.     ausgelöst je Loch (Bombe/Klebe/Tausch)  Schild fing  Störfelder gelegt/eingesammelt je Loch');
const kosten: string[] = [];
for (const stufe of stufen) {
  const mit = leer();
  const ohne = leer();
  for (let saat = 1; saat <= saaten; saat += 1) {
    spiele(saat * 7919, stufe, true, mit);
    spiele(saat * 7919, stufe, false, ohne);
  }
  ...
  const ab = (m: Messung): string => (m.abstand / saaten).toFixed(2);
  console.log(
    `${stufe.padEnd(11)} ${`${sb(ohne)} → ${sb(mit)}`.padEnd(18)} ${`${ab(ohne)} → ${ab(mit)}`.padEnd(17)} ${`${je(mit.ausgeloest.bombe ?? 0)} / ${je(mit.ausgeloest.klebefeld ?? 0)} / ${je(mit.ausgeloest.tausch ?? 0)}`.padEnd(39)} ${je(mit.abgewehrt).padEnd(12)} ${je(mit.gelegt)} / ${je(mit.eingesammelt)}`,
  );
