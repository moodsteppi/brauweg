...
test(`Simulation: ${PARTIEN} Partien mit gleichverteilt gewaehlten Aktionen`, () => {
  ...
  /*
   * Ohne diese Zusicherung koennte die Simulation still zu lauter
   * Normalspielen verkommen und niemandem fiele es auf.
   *
   * Die Hochzeit steht hier bewusst NICHT namentlich: Sie setzt voraus, dass
   * ein Sitz beide Kreuz-Damen haelt, und das ist selten — nachgemessen ueber
   * 400 Gaben zu viert wird sie in 94 von 1600 Sitz-Gelegenheiten ueberhaupt
   * angeboten, knapp sechs Prozent. Bei der hier gefahrenen Partienzahl faellt
   * sie deshalb regelmaessig aus, und eine Zusicherung, die vom Blatt abhaengt,
   * ist keine Zusicherung, sondern ein Wuerfel. Verlangt wird stattdessen, was
   * der Zweck ist: Es darf nicht bei Normalspielen bleiben. Die Hochzeit
   * bekommt einen eigenen, gestellten Fall (regression.test.ts fuehrt es fuer
   * die Armut vor).
   */
  assert.ok(spielarten.has('normal'), 'Spielart normal kam in keiner Partie vor');
  assert.ok(
    [...spielarten].some((art) => art !== 'normal'),
    `nur Normalspiele in ${PARTIEN} Partien — die Simulation erreicht die Vorbehalte nicht mehr`,
  );
  assert.ok(
