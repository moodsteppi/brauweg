...
| Haupthaus im Wald −20 % erlittener Schaden, mit Anzeige | **Umgesetzt 7.8., Anzeige komplett.** Regel in `trefferAuf`; Einheitenwerte-Tafel; in der Aufstellung steht der Hinweis „−20 % erlittener Schaden" auf einem Waldfeld der eigenen Hälfte (neben dem Erdwärme-Hinweis) |
| Haupthaus Stufe 3 auch durch zwei Stützpunkte auf Stufe 2 (z. B. Mauer + Schützenturm) | **Umgesetzt 7.8.** `hausStufe` zählt Stützpunkte: einer → Stufe 2, zwei → Stufe 3; ein Werk/eine Mauer auf Stufe 3 bleibt der Einzelweg. Gezählt werden Objekte, nicht Felder — ein 1×2-Werk am Haus zählt einmal |
| Schützenturm auch im Wald: +25 % Schaden, −1/3 erlittener Schaden, +4 Ressourcen | **Umgesetzt 7.8.** Ein Bogen im Wald wird zum Turm (unbeweglich, zählt zu den drei Schützen-Stellungen). +25 % Schaden und +50 % Leben kamen schon vom Wald, neu ist die Drittel-Minderung und der Gerüstaufschlag +4 (Fels kostet weiter +5). **Beachten:** Zusätzlich greift der bestehende Stellungsbonus −12,5 % für alles, was steht — 12 Schaden werden dadurch zu 7, nicht zu 8. Kein `berg`, also kein brennendes Wrack beim Fall |
| Pfeil des Schützen als sichtbares Geschoss mit Partikeln | **Umgesetzt 7.8.** Der `arrow`-Effekt bestand schon im Kern, wurde in 3D aber nie gezeichnet; jetzt Schaft und Spitze entlang der Flugbahn plus Leuchtspur (Funken an festen Zeitschwellen, gleiche Dichte bei jeder Bildrate) |
| Geländeeffekt der gezogenen Karte auf dem Zielfeld anzeigen | **Umgesetzt 7.8.** `bonusInfo` je Kartenart × Gelände, angezeigt auch dann, wenn (noch) nicht gebaut werden kann — die Farbe sagt weiter, ob es geht (rot = hier nicht). Gilt in 2D und 3D über dieselbe `schildListe` |
